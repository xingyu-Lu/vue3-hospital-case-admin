<template>
	<el-card style="min-height: 100%;">
		...
		<el-table v-loading="loading" :data="tableData" stripe style="width: 100%">
			...
			<el-table-column prop="stomach_ca_id" label="胃ca id" />
...
<script>
	...
	export default {
		...
		setup() {
			...
			onMounted(() => {
				getStomachCa()
				getStomachFollowUpList()
			})
			const getStomachCa = () => {
				axios.get(`/api/back/stomachCas/${stomach_ca_id}`).then(res => {
					state.stomachCa = res.data
				})
			}
			
			// 获取admin列表
			const getStomachFollowUpList = () => {
				state.loading = true
				axios.get('/api/back/stomachFollowUps', {
					params: {
						page: state.currentPage,
						page_size: state.pageSize,
						stomach_ca_id: stomach_ca_id
					}
				}).then(res => {
